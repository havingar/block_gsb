        ...
        if ($viewgsb == 'Yes') {
            ...
            if ($automedal == '1') {
                ...
                if ($config->subcategories == '1') {
                    $sql = "SELECT DISTINCT {course}.id, {role_assignments}.roleid, COUNT({role_assignments}.roleid) AS studentsenrolled
                            FROM {course}
                            JOIN {role_assignments} ra ON ({role_assignments}.contextid = :ctxid)
                            WHERE {course}.id = :cid AND {role_assignments}.roleid $insql
                            HAVING COUNT({role_assignments}.roleid) > :enrolments";
                } else {
                    $sql = "SELECT DISTINCT {course}.id, {role_assignments}.roleid, COUNT({role_assignments}.roleid) AS studentsenrolled
                            FROM {course}
                            JOIN {course_categories} ON ({course_categories}.id = {course}.category)
                            JOIN {role_assignments} ON ({role_assignments}.contextid = :ctxid)
                            WHERE {course}.id = :cid AND {role_assignments}.roleid $insql AND {course_categories}.depth = 1
                            HAVING COUNT({role_assignments}.roleid) > :enrolments";
                }
				...
				if (!empty($getcourseids)) {
	                foreach ($getcourseids as $row => $values) {
	                    ...
	                    if ($test < 1) {
	                        ...
	                        $record->labelsnum     = 0;
	                        $DB->insert_record('block_gsb', $record);
	                    }
	                }
	                ...
	                $sql = "SELECT {block_gsb}.id AS gb, {course}.id, {block_gsb}.ids, {course}.shortname, {course}.fullname, {block_gsb}.gsb, {block_gsb}.enrolnum, {block_gsb}.gsboverride
							FROM  {course}
							INNER JOIN {block_gsb} ON {course}.id = {block_gsb}.ids
	                ...
	                foreach ($get_dept_codes as $row => $values) {
	                    ...
	                    $sql     = "SELECT {context}.id
	                                FROM  {context}
	                                WHERE {context}.contextlevel = '$level' AND {context}.instanceid = '$courseid'";
